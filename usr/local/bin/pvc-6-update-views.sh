...
#  echo "SAN:$SAN HOST:$HOST DATASET:$DATASET SANVOLNAME:$SANVOLNAME OSMAPNAME:$OSMAPNAME OSMOUNT:$OSMOUNT BASEVOL:$BASEVOL DBNAME:$DBNAME"
  ...
  if [ "$MYSQLDATADIR" = "." ] || [ "$DBNAME" = "." ] ; then
    echo "Not updating views because either MYSQLDATADIR or DBNAME is blank"
    NOLINK=1
  else 
    if ping -i 0.2 -c1 -w1 ${HOST} 2>/dev/null >/dev/null ; then
      if [ "$TESTONLY" ]; then
	echo "TESTONLY: not updating views on $HOST"
        RESP=$(ssh ${HOST_SSHOPTS} ${HOST} "if cd ${MYSQLDATADIR}/${DATASET} && cd ${MYSQLDATADIR}/${DBNAME}; then echo 'TEST: update_skeleton_db.sh ${DATASET} ${VERSION}'; which update_skeleton_db.sh >/dev/null; else echo "ERROR: $DATASET and $DBNAME databases must both exist"; fi " )
      else
        RESP=$(ssh ${HOST_SSHOPTS} ${HOST} "if cd ${MYSQLDATADIR}/${DATASET} && cd ${MYSQLDATADIR}/${DBNAME}; then echo -n 'Updating views on ${DATASET} -> '; update_skeleton_db.sh ${DATASET} ${VERSION} ; else echo "ERROR: $DATASET and $DBNAME databases must both exist"; fi " )
      fi
      echo "$RESP"
